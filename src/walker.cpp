 ...
 * State-Design Pattern. This class defines the interface for all concrete states
 * 
 */
class RoombaState {
    public:
        ...
        /**
         * @brief Function to get the next state after following the current state based on LiDAR Data
         * 
         * @param msg The input laser scan data which will determine if the obstacle is near or not.
         */
...
public:
    ...
    /**
     * @brief the virtual method being overidden for the rotation behaviour of the robot
     * 
     * If distance between the object and robot is less than 0.7m, the the robot switches to 
     * rotation state while keeping the linear velocity constant
     * 
    ...
    /** 
     * @brief Changes the next to back to forward state after the rotation is completed
     * 
...
private:
    static bool toggle_rotation_;  ///< Static toggle flag for alternative rotation directions
};
...
public:
    ...
    /**
     * @brief Determining the next state after the forward direction detectts the objects
     * 
     * The robot transitions to rotating state if the obstacle is detected
     * 
     * @param msg Uses the laser data to detect the object and returns the shared pointer to 
     */
    std::shared_ptr<RoombaState> getNextState(const sensor_msgs::msg::LaserScan::SharedPtr msg) override {
        if (msg->ranges[0] < 0.6) {
...
// Implementing the RotatingState::getNextState method after Forward State is finished
std::shared_ptr<RoombaState> RotatingState::getNextState(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
    if (msg->ranges[0] >= 0.7) {
        ...
        return std::make_shared<ForwardState>(); ///< Move to forward state after rotation state
    }
    return nullptr;
}

/**
 * @brief Roomba class manages the transition between current states and next states
 */
class Roomba {
public:
    /**
    ...
    void update(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
        ...
        if (next_state) {
            current_state_ = next_state; ///< Transition to the next state from current state
        }
    }
...
private:
    std::shared_ptr<RoombaState> current_state_; ///< Initiallizing the member variable for current state
};

/**
 * @brief ROS2 Node that manages the velocity publisher and subscribes to the lidar data
 */
class RoombaNode : public rclcpp::Node {
    ...
    private:
        /**
         * @brief Callback function which processes the lidar data and updates the linear and 
         * angular velocity 
         */
    ...
    rclcpp::Subscription<sensor_msgs::msg::LaserScan>::SharedPtr laser_sub_;    ///< Laser Subscriber
    geometry_msgs::msg::Twist twist_msg_;   ///< velocity attribute to publish the message
